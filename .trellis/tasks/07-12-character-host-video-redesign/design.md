...
- **Script:** retains three panel-local scroll regions: newest-first LLM requests, one shared middle detail inspector, and the compact Script Author console. The right console exposes Director review as a selector instead of permanently rendering its body. Selecting it replaces the middle LLM Details body with the full candidate review and approval actions; selecting any request restores its details without a modal.
