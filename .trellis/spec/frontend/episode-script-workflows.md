...
- All three panel bodies own their vertical overflow with `min-h-0` plus `overflow-y-auto`. Request and detail headers remain visible, including when there are zero requests. `LLM requests` renders newest first as full-row selection buttons, defaults selection to the newest request, and uses `aria-pressed` for the active row. `LLM Details` renders only the selected request's shared Prompt/Token/output content inline; Script must not render a details icon or `<dialog>`. Editor and Timeline may continue using the same shared content inside their existing modal trigger.
