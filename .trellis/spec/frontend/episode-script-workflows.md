...
- The Script page is a fixed-height tool workspace: the page root uses `h-screen overflow-hidden`, the game header occupies its own fixed row, and the remaining area is a `min-h-0` responsive grid. On desktop, 「剧本控制台」 is the left status/action panel and `LLM requests` is the right audit panel; narrow viewports stack the same two bounded panels. “Locked” describes the pre-approval Game invariant and warning copy, not the control panel's name.
...
Correct: project requests once at the page boundary, render one fixed audit panel beside the locked control panel, and give each panel body its own bounded overflow.
