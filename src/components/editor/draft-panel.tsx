...
import {
  AutoConfirmDraftButton,
  LlmGenerationNotification,
  LlmNotificationPermissionButton,
...
type DraftPanelProps = {
  ...
  readonly scriptedStructureLocked?: boolean;
};
...
export function DraftPanel({
  ...
  const llmDraft =
    isLlmDraft(draft) &&
    (!scriptedStructureLocked || isLlmSpeechDraft(draft));
  const generatingInitialDraft = llmDraft && !latestGeneration;
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        <div className="px-3 py-3">
          <div className="flex flex-wrap gap-1.5">
            ...
            {speechBudget && isLlmSpeechDraft(draft) ? (
              <MetaPill>
                {speechBudget.tier} · {evaluateSpeech(draft.payload.text, speechBudget).characterCount}/
                {speechBudget.hardMaxCharacters} 字
              </MetaPill>
            ) : null}
          </div>
        </div>
        ...
        {actorBrief ? (
          <section className="border-t border-border px-3 py-4">
            <SectionLabel>Scripted scene</SectionLabel>
              ...
              <div className="flex shrink-0 gap-2">
                <form action={regenerateDraftAction.bind(null, gameId)}>
                  <FormSubmitButton
                    ...
                    ariaLabel={generatingInitialDraft ? "Generating draft" : "Regenerate"}
                    title={generatingInitialDraft ? "Generating draft" : "Regenerate"}
                    forcePending={generatingInitialDraft}
                    className={iconButtonClassName()}
                    llmGenerationNotification={{
                      gameId,
                      draftId: draft.id,
                      previousGenerationId: latestGeneration?.id ?? null,
                    }}
                  />
                </form>
                {latestGeneration ? (
        ...
        {scriptedStructureLocked && !isLlmSpeechDraft(draft) ? (
          <section className="border-t border-border px-3 py-4 text-xs leading-5 text-muted">
            此行动由已批准剧本锁定，不能修改目标或选择。
          </section>
        ) : (
          <DraftPayloadForm gameId={gameId} draft={draft} players={players} alivePlayerIds={alivePlayerIds} />
        )}
        <DraftDecisionActions gameId={gameId} draftId={draft.id} />
      </div>
      {generatingInitialDraft ? (
        <AutoGenerateDraft gameId={gameId} draftId={draft.id} />
      ) : null}
    </section>
  );
}
...
function DraftDecisionActions({
  gameId,
  draftId,
}: {
  readonly gameId: GameId;
  readonly draftId: DraftEvent["id"];
}) {
  return (
    <div className="flex items-center gap-2 border-t border-border px-3 py-3">
      <form action={confirmDraftAction.bind(null, gameId, draftId)}>
        <FormSubmitButton
...
function DraftPayloadForm({
  ...
  const controls = renderDraftPayloadControls(draft, players, alivePlayerIds);
...
function renderDraftPayloadControls(
  ...
  switch (draft.type) {
    ...
    case "wolf_vote_resolved":
      return draft.payload.targetPlayerId === null ? (
        <PlayerTargetField
          label="平票人工裁定"
          players={players.filter((player) =>
            draft.payload.tiedTargetPlayerIds.includes(player.playerId),
          )}
          defaultValue=""
          includeEmptyOption
        />
      ...
      return (
        <label className="block">
          ...
          <textarea
            ...
            rows={4}
            className="mt-1 w-full resize-none rounded-md border border-interactive-border bg-background px-3 py-2 text-sm leading-6 text-foreground outline-none transition placeholder:text-subtle"
          />
        </label>
      );
  ...
  return (
    <label className="block">
      ...
      <select
        ...
        className="mt-1 w-full rounded-md border border-interactive-border bg-background px-3 py-2 text-sm text-foreground outline-none transition"
      >
