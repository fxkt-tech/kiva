...
const systemVoiceRoutePrefix = "/kivdb-assets/voice/system";

export async function loadSystemVoiceDurations(
  dataDir = "kivdb",
): Promise<ReadonlyMap<string, number>> {
  const voiceDir = join(dataDir, "assets", "voice", "system");
  const files = await readdir(voiceDir);
  const entries = await Promise.all(
    files
      .filter((file) => file.endsWith(".mp3"))
      .map(async (file) => {
        const data = await readFile(join(voiceDir, file));
        return [
          `${systemVoiceRoutePrefix}/${file}`,
          Math.ceil(mp3DurationMs(data)),
        ] as const;
      }),
  );
